...
sys.path.insert(0, os.path.abspath("../name-validation-2-python/SOAP"))

from name_info_v2_soap import NameInfoV2Soap


def get_name_info_v2_soap_sdk_go(is_live: bool, license_key: str) -> None:
    print("\n-----------------------------------------")
    print("Name Validation 2 - NameInfoV2 - SOAP SDK")
    print("-----------------------------------------")
    ...
    try:
        service = NameInfoV2Soap(
            license_key,
            is_live,
            timeout_ms=timeout_seconds * 1000
        )
        response = service.get_name_info(name, option)

        if not hasattr(response, "Error"):
            print("\n* Name Info *\n")
            if hasattr(response, "NameInfoV2") and response.NameInfoV2:
                ...
                print("\n* Best Guess Name *\n")
                if hasattr(response.NameInfoV2, 'BestGuessName') and response.NameInfoV2.BestGuessName:
                    print(f"Prefix     : {getattr(response.NameInfoV2.BestGuessName, 'Prefix', '')}")
                    print(f"First Name : {getattr(response.NameInfoV2.BestGuessName, 'FirstName', '')}")
                    print(f"Middle Name: {getattr(response.NameInfoV2.BestGuessName, 'MiddleName', '')}")
                    print(f"Last Name  : {getattr(response.NameInfoV2.BestGuessName, 'LastName', '')}")
                    print(f"Suffix     : {getattr(response.NameInfoV2.BestGuessName, 'Suffix', '')}")
                else:
